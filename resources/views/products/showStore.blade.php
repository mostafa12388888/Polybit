            ...
            <div class="flex gap-x-4 gap-y-3 flex-wrap text-sm items-center">
                <div class="flex gap-2 items-center">
                    <a href="{{ route('store-categories.show', $category) }}"
                        class="text-sm font-light line-clamp-1">{{ $category->name }}</a>
                </div>
            </div>
        @endif
    </x-slot>
        ...
        <div class="container mx-auto flex flex-col gap-20">
            <div class="flex flex-col lg:flex-row gap-8">
                ...
                <div class="flex-grow flex flex-col gap-6 mx-4 sm:mx-0 mt-4">
                    <h2 class="font-bold text-4xl xl:text-[2.5rem] text-dark-800 dark:text-dark-100 leading-tight">
                        {{ $product->name }}</h2>
            ...
            @php
                $specs = $product->specs
                    ->filter(function ($spec) {
                        return in_array(app()->getLocale(), $spec->locales ?? []) ||
                            $spec->media->where('pivot.type', app()->getLocale())->count();
                    })
                    ->sortBy('order');
            @endphp
                ...
                <div class="max-sm:px-4 lg:rounded-md overflow-hidden" x-data="{
                    ...
                    <nav class="w-full flex max-lg:hidden bg-secondary-50 dark:bg-dark-700/20">
                        @foreach ($specs as $spec)
                            <x-button styling="light-link"
                                class="!shadow-none rounded-none flex-grow py-5 border-b border-dark-200 dark:border-dark-600/50 !rounded-t-md bg-transparent {{ count($specs) < 4 ? 'max-w-sm' : '' }}"
                                x-bind:class="{ '!bg-white border border-b-0 dark:!bg-dark-700/40 dark:focus:!brightness-100': isActive(
                                        {{ $loop->index + 1 }}) }"
            ...
            <div
                class="bg-secondary-50 dark:bg-dark-700/60 p-4 md:p-6 lg:rounded-md gap-2 dark:border-dark-700 flex flex-wrap justify-between items-center">
                <h3 class="uppercase font-semibold text-lg text-gray-800 dark:text-dark-100">
                    {{ __('share this product') }}</h3>
