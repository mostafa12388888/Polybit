<div class="lg:p-6 lg:bg-white lg:dark:bg-dark-700/40 flex flex-col rounded-b-md lg:border lg:border-t-0 border-dark-200 dark:border-dark-600/50 overflow-hidden">
    @foreach ($specs as $spec)
        <div class="rounded-md overflow-hidden mb-2 lg:mb-0 border border-secondary-300 dark:border-dark-700/40">
            <x-button styling="light" class="!bg-secondary-200/50 lg:hidden !shadow-none rounded-none flex-grow py-4 dark:bg-dark-700/60 dark:focus:!brightness-100 w-full border-dark-400 dark:border-dark-700 flex items-center gap-2 justify-between"
                ...
                <span class="text-2xl" x-text="tab == {{ $loop->index + 1 }} ? '-' : '+'"></span>
            </x-button>
            
            <div class="lg:!duration-0 lg:!transition-none  border-t-0 lg:border-none rounded-b-md" x-show="tab == {{ $loop->index + 1 }}" x-collapse>
                        ...
                        @foreach ($spec->media->where('pivot.type', app()->getLocale()) as $file)
                            <div class="flex gap-4 flex-wrap items-center justify-between rounded-md lg:p-4">
                                ...
                                <div class="flex w-auto gap-2 flex-wrap">
                                    <x-link href="{{ $file->getSignedUrl() }}" target="_blank" styling="light" class="flex items-center justify-center gap-2 flex-grow dark:lg:!bg-dark-600  max-lg:rounded-md max-lg:aspect-square max-lg:p-1.5">
