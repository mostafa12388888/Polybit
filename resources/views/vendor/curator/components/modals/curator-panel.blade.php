<div
    x-data="{
        handleItemClick: function (mediaId = null, event) {
            ...
            if (Object.values($wire.selected).length === 1 && $wire.selected[0].id != mediaId) {
        ...
        isSelected: function (mediaId = null) {
            if (Object.values($wire.selected).length === 0) return false;
    ...
    <div class="curator-panel-toolbar px-4 py-2 flex items-center justify-between bg-gray-200/70 dark:bg-black/20 dark:text-white">
        <div class="flex items-center gap-2">
            <x-filament::button
                ...
                x-show="Object.values($wire.selected).length > 1"
            >
        ...
        <div class="curator-panel-gallery flex-1 h-full overflow-auto p-4">
            <ul class="curator-picker-grid">
                @forelse ($files as $file)
                    <li
                        wire:key="media-{{ $file['id'] .'-'. uniqid() }}" class="relative aspect-square"
                        x-bind:class="{
                            'opacity-40': Object.values($wire.selected).length > 0 && !isSelected('{{ $file['id'] }}')
                        }" 
                    >
