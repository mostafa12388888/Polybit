                    ...
                    @foreach ($projects as $project)
                        <li class="glide__slide h-full relative">
                            <x-link :href="route('projects.show', $project)" class="group flex flex-col text-start !p-0 bg-white dark:bg-dark-700/70 dark:shadow-none border dark:border-none !rounded-md overflow-hidden">
                                <div class="relative">
                                    <x-curator-glider fallback="logo" :media="$project->image" format="webp" width="480" height="280" fit="fill" bg="FFFFFF" quality="70" class="w-full max-sm:aspect-video sm:h-44 md:h-56 lg:h-72 xl:h-56 2xl:h-72 object-cover" :alt="$project->title" />
