<div class="bg-gradient-to-b focus:border-secondary-200 dark:from-dark-700/40 dark:to-dark-700/40 sm:px-6 pt-12 pb-5 md:pt-16 xl:pt-24 relative">
                    ...
                    @foreach ($store_categories->take(6) as $store_category)
                        <li class="glide__slide h-full">
                            <x-link href="#" class="group flex items-center text-start gap-3 !px-3 !py-10 aspect-video bg-white dark:bg-dark-900/40 shadow-md shadow-dark-200 dark:shadow-none !rounded-xl mx-2 my-10 overflow-hidden relative" :href="route('store-categories.show', $store_category)">
                                <x-curator-glider fallback="logo" :media="$store_category->image" format="webp" width="480" height="280" fit="crop" quality="70" class="absolute top-0 left-0 w-full h-full flex-grow object-cover transition-transform group-hover:scale-125" :alt="$store_category->name" />
                                ...
                                <div class="flex-grow flex flex-col gap-3 py-2 h-28 relative z-10 text-white text-center items-center justify-center drop-shadow-[0_0_15px_rgba(0,0,0,.6)]">
                                    ...
                                    <p>{{ __(':products_count Products', ['products_count' => $store_category->sub_categories_products()->count()]) }}</p>
                                </div>
                            </x-link>
                        </li>
                    @endforeach
