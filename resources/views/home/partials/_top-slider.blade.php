<div class="glide overflow-hidden min-[2561px]:rounded-2xl min-[2561px]:my-8 min-[2561px]:container min-[2561px]:mx-auto relative" x-init="$nextTick(() => new Glide($el, {
    type: 'carousel',
    direction: '{{ direction() }}',
    autoplay: 3000,
}).mount());">
<div data-glide-el="track" class="glide__track">
        <ul class="glide__slides h-72 sm:h-80 md:h-96 lg:h-[30rem] xl:h-[34rem]">
            @foreach (range(1,3) as $slide)
                <li class="glide__slide h-full relative">
                    <div class="w-full h-full">
                        <div class="absolute w-full h-full top-0 left-0 bg-dark-900/60 pointer-events-none"></div>
                        ...
                        <div class="absolute  w-full h-full p-8 md:p-24 flex flex-col gap-4 md:gap-6 xl:gap-8 items-center justify-center text-center">
                            <h2 class="text-gray-50 dark:text-dark-50 text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-extrabold">{{ str()->title(fake()->sentence(3)) }}</h2>

                            <p class="text-gray-50 dark:text-dark-50 text-base md:text-lg xl:text-xl text-ellipsis w-full max-w-6xl overflow-hidden">{{ fake()->paragraph(3) }}</p>
                        ...
                        <img src="{{ asset('storage/slide' . $slide . '.webp') }}" class="w-full h-full object-cover" alt="">
                    </div>
                </li>
            @endforeach
        </ul>
    ...
    <div class="glide__bullets max-md:hidden" data-glide-el="controls[nav]">
        <button class="glide__bullet dark:bg-dark-300" data-glide-dir="=0"></button>
        <button class="glide__bullet dark:bg-dark-300" data-glide-dir="=1"></button>
        <button class="glide__bullet dark:bg-dark-300" data-glide-dir="=2"></button>
    </div>
</div>
