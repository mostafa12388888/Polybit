<x-link href="#" class="flex-grow group flex max-sm:flex-col gap-6 text-start items-start !p-0 shadow-none overflow-hidden !rounded-none" :href="route('posts.show', $post)">
    <div class="relative max-sm:w-full flex-shrink-0 rounded-md overflow-hidden">
        <x-curator-glider fallback="logo" :media="$post->image" format="webp" width="480" height="280" fit="contain" quality="70" class="w-full aspect-[8/5] sm:aspect-[8/7] md:aspect-[8/5] sm:w-64 md:w-96 lg:w-96 object-contain bg-white" :alt="$post->title" loading="{{ ($lazy ?? null) ? 'lazy' : 'eager' }}" />
